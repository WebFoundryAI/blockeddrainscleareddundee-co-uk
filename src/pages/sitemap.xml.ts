...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Dundee gets higher priority)
    const isDundee = location.slug === 'dundee';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isDundee ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isDundee ? 1 : 2 + i),
    });
